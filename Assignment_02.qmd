...
topic_keys_list = c("VRDTHR","MEODR", "INC", "HCSNSP")
topic_cols_list = c("all_mortality", "opioid_mortality","md_hh_income","perceived_safety")
period_keys_list = c("2023", "2019-2023", "2023-2024")
## END EDIT
...
Description of mortality indicators, maps and charts. Include why you chose the indicators, what they show, and any other relevant information.

## Row {height=60%}
### Column
```{r}
#| label: Mortality Map 1
#| title: All-Cause Mortality Rate (per 100,000), 2023 # EDIT HERE
#| eval: true

# Create custom choropleth map using specified parameters
# BEGIN EDITING
var <- "all_mortality" # specify variable to map
map_color <- "Blues" # specify color range for map
map_title <- "Mortality Rate" # specify map title
# STOP EDITING
...
### Column
```{r}
#| label: Mortality Map 2
#| title: Opioid-Related Overdose Mortality Rate (per 100,000), 2023 # EDIT HERE
#| eval: true

# Create custom choropleth map using specified parameters
# BEGIN EDITING
var <- "opioid_mortality" # specify variable to map
map_color <- "Blues" # specify color range for map
map_title <- "Mortality Rate" # specify map title
# STOP EDITING
...
### Column
```{r}
#| label: Mortality Histogram 1
#| title: Histogram
#| eval: true

df <- community_area_data_table %>%
  st_set_geometry(NULL) %>%
  transmute(all_mortality = as.numeric(all_mortality)) %>% # EDIT HERE (two values)
  filter(is.finite(all_mortality)) # EDIT HERE

plot_ly(
  df,
  x = ~all_mortality, # EDIT HERE
  type = "histogram",
  nbinsx = 20,
...
df <- community_area_data_table %>%
  st_set_geometry(NULL) %>%
  transmute(opioid_mortality = as.numeric(opioid_mortality)) %>% # EDIT HERE (two values)
  filter(is.finite(opioid_mortality)) # EDIT HERE

plot_ly(
  df,
  x = ~opioid_mortality, # EDIT HERE
  type = "histogram",
  nbinsx = 20,
  marker = list(color = "#3182bd",
                line = list(color = "rgba(0,0,0,0.25)", width = 0.5))
...
Description of social determinants of health (SDOH) indicators, maps and charts. Include why you chose the indicators, what they show, and any other relevant information.
...
```{r}
#| label: SDOH Map 1
#| title: Median Household Income, 2019-2023 # EDIT HERE
#| eval: true

# Create custom choropleth map using specified parameters
# BEGIN EDITING
var <- "md_hh_income" # specify variable to map
map_color <- "Oranges" # specify color range for map
map_title <- "Household Income" # specify map title
# STOP EDITING
...
```{r}
#| label: SDOH Map 2
#| title: "Perceived Neighborhood Safety Rate, 2023-2024" # EDIT HERE
#| eval: true

# Create custom choropleth map using specified parameters
# BEGIN EDITING
var <- "perceived_safety" # specify variable to map
map_color <- "Oranges" # specify color range for map
map_title <- "Perceived Safety" # specify map title
# STOP EDITING
...
df <- community_area_data_table %>%
  st_set_geometry(NULL) %>%
  transmute(md_hh_income = as.numeric(md_hh_income)) %>% # EDIT HERE  (two values)
  filter(is.finite(md_hh_income)) # EDIT HERE

plot_ly(
  df,
  x = ~md_hh_income, # EDIT HERE
  type = "histogram",
  nbinsx = 20,
  ...
  layout(
    xaxis = list(title = "Income"), # EDIT HERE
    yaxis = list(title = "Count"),
    bargap = 0.05,
    showlegend = FALSE
  )
...
df <- community_area_data_table %>%
  st_set_geometry(NULL) %>%
  transmute(perceived_safety = as.numeric(perceived_safety)) %>% # EDIT HERE (two values)
  filter(is.finite(perceived_safety)) # EDIT HERE

plot_ly(
  df,
  x = ~perceived_safety, # EDIT HERE
  type = "histogram",
  nbinsx = 20,
  marker = list(color = "#A63603",
                line = list(color = "#FDBE85", width = 0.5))
  ...
  layout(
    xaxis = list(title = "Rate (%)"), # EDIT HERE
    yaxis = list(title = "Count"),
    bargap = 0.05,
    showlegend = FALSE
  )
...
vars <- c("all_mortality", "opioid_mortality", "md_hh_income", "perceived_safety") # EDIT HERE
...
var = "all_mortality"  # EDIT HERE: choose one variable from vars
...
```{r}
#| title: Opioid Mortality 
#| eval: true
#| echo: false

# Create custom LISA map using specified variable
var = "opioid_mortality"  # EDIT HERE: choose one variable from vars
...
# Create custom LISA map using specified variable
var = "md_hh_income"  # EDIT HERE: choose one variable from vars
...
# Create custom LISA map using specified variable
var = "perceived_safety"  # EDIT HERE: choose one variable from vars
...
2. **Geographic Patterns and Trends**  
